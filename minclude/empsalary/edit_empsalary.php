...
<?php
	if(isset($_POST["txtInid"])){
		...
		$income_table = $db->query("select id,employee_id,source,billdate,paidamount,dueamount,status from b_exp where id='$in_id'");
	...
	if(isset($_POST["btnUpdate"])){
		...
		$db->query("update b_exp set employee_id='$empid', source='$source',billdate='$date',paidamount='$pamount',dueamount='$damount',status='$status' where id='$in_id'");
...
<div><a style="text-decoration:none; float:right" href="home.php?item=36" class="btn btn-success"><i class="glyphicon glyphicon-list"></i> Employee Salary List</a></div>

<form method="post" action="home.php?item=38" class="form-horizontal">
    ...
    <div class="col-sm-5">
    <input type="text" name="txtSource" class="form-control" placeholder="Source Name" value="<?php echo isset($source)?$source:""?>"/>
